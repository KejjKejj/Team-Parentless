...
public class TutorialGlock : Weapon
{
    ...
    void OnTriggerStay2D(Collider2D collissionobject)
    {
        if (collissionobject.tag == "Player")
        {
            if (Input.GetButton("Weapon") && !IsPickedUp && gameObject.tag == "Weapon" && PickUpDelayTimer >= DropDelay &&
                !collissionobject.GetComponent<TutorialMovment>().CarryingWeapon)
            {
                Player = collissionobject.GetComponent<TutorialMovment>();
                PickUpWeapon();
            }
        }
    }
    ...
    public void PickUpWeapon()
    {
        ...
        GameObject.Find("Character").GetComponent<TutorialMovment>().WeaponDamage = damage;
    ...
    void Update()
    {
        ...
        if (!Automatic)
        {
            if (IsPickedUp && FireRateTimer >= FireRate && CurrentAmmo > 0)
            {
                ...
                {
                    Instantiate(TutorialBullet, transform.position, transform.rotation);
                    FireRateTimer = 0;
                    CurrentAmmo--;
